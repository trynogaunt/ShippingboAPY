...
class OrderDispatch(BaseModel):
    ...
    state: Literal["waiting_for_dispatch", "need_redispatch", "dispatched", "in_trouble", "partially_shipped", "shipped", "canceled"] = Field(..., alias="state", description="The current state of the order dispatch (e.g., waiting_for_dispatch, need_redispatch, dispatched).")
    ...
    supplier_orders: Optional[List[dict[Any, Any]]] = Field(None, alias="supplier_orders", description="The list of supplier order IDs associated with the order dispatch, if applicable.")
