from shippingboapy.auth import get_token
import pytest
import httpx

@pytest.mark.asyncio
async def test_get_token(mock_config):
    async with httpx.AsyncClient() as mock_session:
        ...
        mock_response = await get_token(auth_code="test_auth_code", session=mock_session, config=mock_config, headers=headers)
